...
%% colorbars:
axes('Position', [xPos(1), yPos(4)-yspacer-0.01, Wsignal, 0.02])
caxis([0 10])
colormap(gca,cMapZscores)
cbr = colorbar('Location', 'south')
cbr.AxisLocation = 'out'
...
axes('Position', [xPos(1)+xspacer+Wsignal, yPos(4)-yspacer-0.01, Wsignal, 0.02])
caxis([0 0.5])
colormap(gca,cMapMotifs2(1:size(cMapMotifs2,1)/2,:))
cbr = colorbar('Location', 'south')
cbr.AxisLocation = 'out'
...
axes('Position', [xPos(1)+2*(xspacer+Wsignal), yPos(4)-yspacer-0.01, Wsignal, 0.02])
caxis([-2 2])
colormap(gca,cMapLogChange)
cbr = colorbar('Location', 'south')
cbr.AxisLocation = 'out'
...
axes('Position', [xPos(1)-Wscheme-7*xspacer yPos(tf)+Hsignal-Hpattern-Hzscore-HfoldChange-12*yspacer/15 Wscheme Hsignal/1.5])
cbr = colorbar('Location','south')
colormap(gca, ccColMap)
set(cbr, 'YTick',[0:0.5:1], 'YTickLabel', [ccTH,ccTH+(1-ccTH)/2, 1],'AxisLocation','out')
ylabel(cbr, 'coiled-coil score', 'fontSize',10)
axis off
...
axes('Position', [xPos(1)+Wsignal/3, yPos(tf)+Hsignal-Hpattern-Hzscore-HfoldChange-12*yspacer/15, Wsignal/3, Hcluster])
caxis([-2 2])
colormap(gca,cMapLogChange)
cbr = colorbar('Location', 'south')
cbr.AxisLocation = 'out'
axis off
ylabel(cbr,'Fold change', 'fontSize',10)
set(cbr, 'Ticks', [0.5+min(caxis),0,max(caxis)-0.1] , 'TickLabels', [min(caxis),0,max(caxis)],'TickLength',[0 0])

axes('Position', [xPos(1)+2*Wsignal/3, yPos(tf)+Hsignal-Hpattern-Hzscore-HfoldChange-12*yspacer/15, Wsignal/3, Hcluster])
caxis([0 4]+0.5)
colormap(gca,cMapMotifs)
cbr = colorbar('Location', 'south')
cbr.AxisLocation = 'out'
axis off
ylabel(cbr,'#motif', 'fontSize',10)
...
Hsignal = 0.1;
xPos = [0.2:xspacer+Wsignal:1-Wsignal];
yPos = [0.85:-(yspacer+Hsignal):0];
Hpattern = (Hsignal-1/5*yspacer)/3;
Hzscore =  (Hsignal-1/5*yspacer)/3+yspacer/15;
HfoldChange =  (Hsignal-1/5*yspacer)/3;
Hcluster = 0.01;
Wscheme = Wsignal/2;
...
for tf = 1:size(zc,1)
    ...
    axes('Position', [xPos(1)-Wscheme-7*xspacer yPos(tf)+Hsignal/2.5 Wscheme Hsignal/1.5])
    for j =1:2
        DBDpos = [ScerDBDs.from_2(find(contains(ScerDBDs.queryName, upper(zc{tf,j})))), ScerDBDs.to_2(find(contains(ScerDBDs.queryName, upper(zc{tf,j}))))];
        idx = find(strcmp(extractfield(ccPredictions.marCoil,'tf'), zc{tf,j}));
        ccPredVec = ccPredictions.marCoil(idx).ccPred;
        [pks, loc, w] = findpeaks(ccPredVec, 'MinPeakHeight', ccTH, 'MinPeakDistance',7);
        w = round(w);
        currL = summaryTable.proteinLength(ismember([summaryTable.p1,summaryTable.p2], zc{tf,j}));
        plot([1 currL], [j j], 'color','k','LineWidth',2)
        hold on
        % plot DBDs
        if tf == 1
            rectangle('Position',[DBDpos(1), j-0.2, diff(DBDpos),0.4], 'Curvature',0.2, 'FaceColor',[0, 0.5, 0], 'lineStyle','none')
        else
            rectangle('Position',[DBDpos(1), j-0.2, diff(DBDpos),0.4], 'Curvature',0.2, 'FaceColor',[1, 0, 1], 'lineStyle','none')
        end
        % coiled-coil
        for c = 1: length(pks)
            rectangle('Position',[loc(c)-w(c)/2, j-0.2, w(c),0.4], 'Curvature',0.2, 'FaceColor',...
                [ccColMap(round((pks(c)-ccTH)./0.01)+1,:)], 'EdgeColor', ccColMap(end,:))
        end
        text(currL+2, j, num2str(currL), 'HorizontalAlignment', 'left', 'VerticalAlignment','middle')
        text(-5, j,  sprintf('%s_{(P%d)}',zc{tf,j},j), 'HorizontalAlignment','right', 'fontSize',14)
    end
    xlim([1 maxL])
    ylim([0.5 2.5])
    set(gca, 'YDir','reverse')
    ...
    % pattern:
    clear nPatternInPromoter
    for p = 1:numel(pattern)
        for g = 1:numel(uTargetIdx)
            currPromoterSeq = findPromoterSeq(uTargetIdx(g), GP, promoterLengthsORF, 'tss', 'ORF');
            currPromoterSeq = upper(currPromoterSeq);
            if tf == 4 & p==1
                currMatches = regexp(currPromoterSeq, pattern{p},'match');
                currMatches = regexp(currMatches, 'CCG...*?CC[AG]|[TC]GG...*?CGG', 'match','once');
                ATcontent = cellfun('prodofsize', regexp(currMatches, 'A|T'));
                nPatternInPromoter(p,g)  = sum(ATcontent./(cellfun('prodofsize', currMatches)-6) >0.7);
            else
            nPatternInPromoter(p,g) = numel(regexp(currPromoterSeq, pattern{p}));
            end
        end
    end
    ...
    % zscore:
    intStrains = intStrains(~contains(intStrains,'_d'));
    lac = allSamples(contains(allSamples, '_lactis') & contains(allSamples, {intStrains{1},intStrains{2}}));
    intStrains = [intStrains, lac];
    zScoresMat = nan(numel(intStrains),6701);
    for j = 1: numel(intStrains)
        zScoresMat(j,:) = nanZscore(checWTdelLactisSwap.sumProm.(intStrains{j}));
    end
    zScoreSelectedTargets =  zScoresMat(:,uTargetIdx);
    ...
    notNanFoldChange = ~isnan(logChangeSelectedTargets);
    % cluster by pattern
    %     clusterIdx = ((nPatternInPromoter(1,:)>0) + (nPatternInPromoter(2,:)>0)*2)+1;
    %     clusterReorder = [4,1,3,2];
    %     clusterIdx = clusterReorder(clusterIdx);
    %     sortBy = [1,1,3,1];
    %     valForSort = nan(size(clusterIdx));
    %     for i = 1:max(clusterIdx)
    %         valForSort(clusterIdx==i) = -zScoreSelectedTargets(sortBy(i),clusterIdx==i);
    %     end
    
    % cluster by targets
    clusterIdx = sum((zScoreSelectedTargets([1,2],:)> zscoreTHclus).*[1;2],1)+1;
    ...
    %axes('Position', [xPos(4)+xspacer+0.012, yPos(tf)+Hsignal+yspacer/15, Wsignal, Hcluster])
    axes('Position', [xPos(1), yPos(tf)+Hsignal+yspacer/15, Wsignal, Hcluster])
    imagesc(clusterIdx(idx))
    set(gca, 'XTick',[],'YTick',[])
    colormap(gca, cMapClusters)
    % caxis([0.5 4.5])
    clusrTextPos = max(xlim)/6+[0, max(xlim)/3, 2*max(xlim)/3];
    if tf ==1
        text(clusrTextPos(1), 0.5, sprintf('\\color[rgb]{%.2f,%.2f,%.2f}Unique P1',cMapClusters(1,:)),...
            'HorizontalAlignment','center','VerticalAlignment','bottom', 'FontWeight','bold')
        text(clusrTextPos(2), 0.5, sprintf('\\color[rgb]{%.2f,%.2f,%.2f}Common',cMapClusters(2,:)),...
            'HorizontalAlignment','center','VerticalAlignment','bottom', 'FontWeight','bold')
        text(clusrTextPos(3), 0.5, sprintf('\\color[rgb]{%.2f,%.2f,%.2f}Unique P2',cMapClusters(3,:)),...
            'HorizontalAlignment','center','VerticalAlignment','bottom', 'FontWeight','bold')
    end
    %axes('Position', [xPos(4)+xspacer+0.012, yPos(tf)+Hsignal-Hpattern-Hzscore-yspacer/15, Wsignal, Hzscore])
    axes('Position', [xPos(1), yPos(tf)+Hsignal-Hpattern-yspacer/15, Wsignal, Hzscore])
    imagesc(zScoreSelectedTargets(:,idx))
    plotgrid(zScoreSelectedTargets(:,idx))
    caxis([0 10])
    YL = regexprep(intStrains, {'_d', '.*_lactis', '_(?=[A-Z])'},{' \\Delta', '\\itK.lac', ' '});
    set(gca, 'YTick', [1:numel(intStrains)], 'YTickLabel', YL,'XTick',[])
    set(gcf, 'Color','w')
    % cbr = colorbar()
    % ylabel(cbr, 'z-score');
    colormap(gca,cMapZscores)
    ...
    %axes('Position', [xPos(4)+xspacer+0.012, yPos(tf)+Hsignal-Hpattern-Hzscore-HfoldChange-2*yspacer/15, Wsignal, HfoldChange])
    axes('Position', [xPos(1), yPos(tf)+Hsignal-Hpattern-Hzscore-yspacer/15, Wsignal, HfoldChange])
    imagesc(logChangeSelectedTargets(find(all(logChangeSelectedTargets,2)),idx))
    plotgrid(logChangeSelectedTargets(find(all(logChangeSelectedTargets,2)),idx))
    caxis([-2 2])
    set(gca, 'YTick', [1,2], 'YTickLabel', intStrains(find(all(logChangeSelectedTargets,2))),'XTick',[])
    % cbr = colorbar()
    % ylabel(cbr, 'z-score');
    colormap(gca,cMapLogChange)
    
    %axes('Position', [xPos(4)+xspacer+0.012, yPos(tf)+Hsignal-Hpattern, Wsignal, Hpattern])
    axes('Position', [xPos(1), yPos(tf)+Hsignal-Hpattern-Hzscore-HfoldChange-2*yspacer/15, Wsignal, Hpattern])
    imagesc(nPatternInPromoter(:,idx))
    plotgrid(nPatternInPromoter(:,idx))
    caxis([0 3])
    %     set(gca, 'YTick', [1:numel(pattern)], 'YTickLabel', pattern,'XTick',[])
    set(gca, 'YTick', [1:numel(pattern)], 'YTickLabel', {'mA','mB'},'XTick',[])
    % set(gca, 'XTick',[1:numel(uTargetIdx)], 'XTickLabel', GP.gene_infoR64.nameNew(uTargetIdx(idx)), 'XTickLabelRotation',90);
    % cbr = colorbar()
    % ylabel(cbr, 'occurrences');
    colormap(gca,cMapMotifs)
    ...
    for g = 1: length(targetList)
        if ~isempty(targetList{g})
            axes('Position', [xPos(g+1)+6*xspacer yPos(tf) Wsignal Hsignal])
            target = targetList{g};
            geneIdx = GP.gene_table.(target);
            pos = GP.gene_infoR64.position(geneIdx,:);
            intBasesGene = pos(2)+[-promoterLengthPlotVec(geneIdx):intoGene].*GP.gene_infoR64.dir(geneIdx);
            ...
            TssLoc = (GP.gene_infoR64.tamarTss(geneIdx,2)-pos(2))*GP.gene_infoR64.dir(geneIdx);
            labelsSignal = {'P1','P1\DeltaP2', 'P2','P2\DeltaP1'};
            if g==1
                %set(gca, 'YTick', [1:numel(labelSamples)], 'YTickLabel', labelSamples(end:-1:1), 'fontSize',12, 'XTick', [-800:200:-400, TssLoc,100], 'XTickLabel', {'-800','-600','-400','TSS', '100'})
                set(gca, 'YTick', [1:numel(labelSamples)+1], 'YTickLabel', {'NucOcc', labelsSignal{numel(labelSamples):-1:1}},'XTick',[], 'fontSize',11)
            else
                set(gca,'YTick',[],'XTick',[])
            end
            ...
            title(sprintf('\\color[rgb]{%.2f,%.2f,%.2f}%s',cMapClusters(g,:),target),'fontSize',9)
            box on
%             ax = gca;
%             ax.XColor = cMapClusters(g,:);
%             ax.YColor = cMapClusters(g,:);
        end
    end
    axes('Position', [xPos(1), yPos(tf)+Hsignal-Hpattern-Hzscore-HfoldChange-7*yspacer/15, Wsignal, Hcluster])
    for g = 1: length(targetList)
        if ~isempty(targetList{g})
            target = targetList{g};
            geneIdx = GP.gene_table.(target);
            currTargetPos = find(uTargetIdx(idx)==geneIdx);
            plot([currTargetPos, currTargetPos], ylim, 'color', cMapClusters(g,:), 'LineWidth', 1.5)
            hold on
            scatter(currTargetPos, max(ylim), [], cMapClusters(g,:), '^','filled')
        end
    end
    xlim(0.5+[0 length(idx)])
    axis off
end
% colorbars:
axes('Position', [xPos(1), yPos(tf)+Hsignal-Hpattern-Hzscore-HfoldChange-12*yspacer/15, Wsignal/3, Hcluster])
caxis([0 10])
colormap(gca,cMapZscores)
cbr = colorbar('Location', 'south')
cbr.AxisLocation = 'out'
...
axes('Position', [xPos(1)-2*Wscheme, yPos(tf)+Hsignal-Hpattern-Hzscore-HfoldChange-12*yspacer/15, Wscheme, Hscatter/15])
cbr = colorbar('Location','south')
colormap(gca, ccColMap)
set(cbr, 'YTick',[0:0.5:1], 'YTickLabel', [ccTH,ccTH+(1-ccTH)/2, 1],'fontSize',8,'AxisLocation','out')
ylabel(cbr, 'coiled coil score', 'fontSize',10)
axis off
